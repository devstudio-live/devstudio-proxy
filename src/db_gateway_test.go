...
func dbPost(t *testing.T, path string, body any) *httptest.ResponseRecorder {
	t.Helper()
...
func TestGateway_UnknownEndpoint(t *testing.T) {
	rr := dbPost(t, "nonexistent", map[string]any{})
...
func TestGateway_StripGatewayHeaders(t *testing.T) {
	// After handleDBGateway processes the request, the gateway headers must be gone.
	// We verify this by checking that an invalid driver returns an app-level error
	// (HTTP 200) not a proxy routing error, confirming the headers were stripped
	// before any forwarding could occur.
	rr := dbPost(t, "test", map[string]any{
...
func TestDescribe_TableRequired(t *testing.T) {
	rr := dbPost(t, "describe", map[string]any{
		"connection": map[string]any{"driver": "sqlite"},
		"table":      "",
	})
	resp := decodeResp(t, rr)
...
func TestRowLimitDefault(t *testing.T) {
	// We can't easily test the limit without a real DB, but we can test
	// that the defaults are applied in the request parsing path.
	var req dbRequest
	req.Limit = 0
	if req.Limit <= 0 {
