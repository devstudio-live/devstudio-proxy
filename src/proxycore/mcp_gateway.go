...
func (s *Server) initMCPRuntime(refreshInterval time.Duration) {
	src, err := s.fetchMCPScript()
	if err != nil {
		log.Printf("mcp: init failed (will fall back to remote): %v", err)
		return
	}

	if err := s.loadMCPScript(src); err != nil {
	...
	if refreshInterval > 0 {
		go s.mcpRefreshLoop(refreshInterval)
	}
}

// fetchMCPScript downloads the MCP routines JS source.
func (s *Server) fetchMCPScript() (string, error) {
...
// active runtime.
func (s *Server) loadMCPScript(src string) error {
	vm := goja.New()
	...
	vm.Set("__storeContext", func(call goja.FunctionCall) goja.Value {
		...
		id, err := s.StoreContext([]byte(jsonStr), "snapshot")
		if err != nil {
			return goja.Null()
		}
	...
	vm.Set("__storeObject", func(call goja.FunctionCall) goja.Value {
		...
		id, err := s.StoreContext(data, "object")
		if err != nil {
			return goja.Null()
		}
		return vm.ToValue(id)
	...
	s.mcpRuntime.vm = vm
	s.mcpRuntime.script = src
	s.mcpRuntime.ready = true
...
func (s *Server) mcpRefreshLoop(interval time.Duration) {
...
func (s *Server) handleMCPGateway(w http.ResponseWriter, r *http.Request) {
	s.mcpRuntime.mu.Lock()
	ready := s.mcpRuntime.ready
	s.mcpRuntime.mu.Unlock()

	if !ready {
		if s.MCPFallbackEnabled {
	...
	s.mcpRuntime.mu.Lock()
	vm := s.mcpRuntime.vm.(*goja.Runtime)
	result, err := callMCPHandleRequest(vm, reqBody)
	s.mcpRuntime.mu.Unlock()
