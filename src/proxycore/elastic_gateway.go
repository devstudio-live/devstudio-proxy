...
func (s *Server) handleElasticGateway(w http.ResponseWriter, r *http.Request) {
	...
	switch path {
	case "test":
		s.handleElasticTest(w, r, req)
	case "query":
		s.handleElasticQuery(w, r, req)
	case "tables":
		s.handleElasticIndices(w, r, req)
	case "describe":
		s.handleElasticDescribe(w, r, req)
	case "databases":
		s.handleElasticDatabases(w, r, req)
	default:
...
func (s *Server) handleElasticTest(w http.ResponseWriter, r *http.Request, req DBRequest) {
	t0 := time.Now()
	client, err := s.getPooledElasticClient(req.Connection)
	if err != nil {
...
func (s *Server) handleElasticQuery(w http.ResponseWriter, r *http.Request, req DBRequest) {
	t0 := time.Now()
	if req.Table == "" {
...
func (s *Server) handleElasticIndices(w http.ResponseWriter, r *http.Request, req DBRequest) {
	t0 := time.Now()
	client, err := s.getPooledElasticClient(req.Connection)
	if err != nil {
		json.NewEncoder(w).Encode(DBResponse{Error: err.Error(), Duration: ms(t0)})
...
func (s *Server) handleElasticDescribe(w http.ResponseWriter, r *http.Request, req DBRequest) {
	t0 := time.Now()
	if req.Table == "" {
		req.Table = req.Connection.Database
	}
...
func (s *Server) handleElasticDatabases(w http.ResponseWriter, r *http.Request, req DBRequest) {
	t0 := time.Now()
	client, err := s.getPooledElasticClient(req.Connection)
	if err != nil {
		json.NewEncoder(w).Encode(DBResponse{Error: err.Error(), Duration: ms(t0)})
